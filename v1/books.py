...
def print_result_table(headers:list, results:list,translated_headers=True):
    """
    返回表格形式，因为pycharm默认控制台对中文的输入并不友好，若想获得完美的表格，请在在pycharm中，运行编辑配置的“修改选项“中勾选”在输出控制台中模拟终端”，或选用vscode
    :param headers: 表头列表
    :param results: 数据库查询结果集（元组列表）
    :param translated_headers: 是否转换表头为中文，默认True
    """
    if not results:
        return "无匹配结果", {}
    ...
    numbered_results = [(i + 1,) + row for i, row in enumerate(results)]
